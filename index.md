---
layout: default
title: Inicio
nav_order: 1
---

# Reportes Académicos — IA Generativa

Portafolio de prácticas del curso de **IA Generativa** (IBERO), impartido por el profesor **Huber Girón**.  
Cada práctica documenta experimentos reales con modelos de lenguaje grande (LLM) ejecutados localmente con **Ollama**.

---

## Prácticas

| # | Práctica | Tema |
|---|----------|------|
| 1 | [LLM locales con Ollama](practica1/) | Instalación, ejecución y comparación de 7 modelos LLM |
| 2 | [Benchmark de LLM](practica2/) | Requerimientos técnicos, parámetros y benchmarking con Python |
| 3 | [Chatbot con LLM Local](practica3/) | Chatbot web con FastAPI y Ollama: arquitectura, métricas y extensión Page Assist |
| 4 | [Copilotos especializados con Ollama](practica4/) | Perfiles de copiloto, comparación genérico vs. especializado y prompt injection |
| 5 | [APIs LLM externas vs Ollama](practica5/) | Ollama local vs. OpenRouter: comparación de costo, latencia y calidad |
| 6 | [Evaluación de arquitecturas LLM + MQTT](practica6/) | Agente LLM clasificador con salida JSON y publicación MQTT: métricas de clasificación |

---

## Proyecto

**[TORQUE](practica-proyecto/)** — Sistema operativo de IA multiagente que administra la operación diaria de un taller mecánico por WhatsApp (texto, voz y fotografía): decide qué acción tomar, la ejecuta sobre los datos del negocio y vigila el taller de forma proactiva. Implementado en **n8n** con un agente orquestador, roles por número de teléfono, ingeniería de prompts por agente y un loop agéntico con límite de iteraciones. [Ver documentación completa de arquitectura →](practica-proyecto/)

---

## Modelos utilizados

| Modelo | Parámetros | Fabricante |
|--------|-----------|-----------|
| TinyLlama 1.1B Chat | 1.1B | TinyLlama |
| Phi-4-mini-instruct | 3.8B | Microsoft |
| Llama 3.2 3B Instruct | 3.21B | Meta |
| Gemma 3 4B IT | 4B | Google DeepMind |
| Qwen2.5 7B Instruct | 7.61B | Alibaba Cloud |
| Mistral 7B Instruct v0.3 | 7B | Mistral AI |
| Qwen3 8B | 8.2B | Alibaba Cloud |

---

## Navegación

- [Práctica 1 – Comparación de modelos LLM](practica1/)
- [Práctica 2 – Benchmark de LLM](practica2/)
- [Práctica 3 – Chatbot con LLM Local](practica3/)
- [Práctica 4 – Copilotos especializados con Ollama](practica4/)
- [Práctica 5 – APIs LLM externas vs Ollama](practica5/)
- [Práctica 6 – Evaluación de arquitecturas LLM + MQTT](practica6/)
- [Proyecto – TORQUE: Sistema Operativo de IA para Talleres Mecánicos](practica-proyecto/)
- [Uso de IA en este sitio](uso-ia/)
